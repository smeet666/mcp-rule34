...
/**
 * Why each builder below stamps its own message.
 *
 * A bound such as `min` raises its issue from the check that carries it, and
 * the issue names that check as the instance whose error map applies. A map set
 * on the schema alone therefore leaves the bounds refusing in zod's own words,
 * with no code for a caller to branch on. So the message is written onto the
 * check itself.
 */
export function choice<const Options extends readonly [string, ...string[]]>(
  argument: string,
  options: Options,
  what: string,
): z.ZodEnum<{ [K in Options[number]]: K }> {
  const error = `${CODE} ${argument} takes one of ${options.join(", ")}. ${what}`;
  return z.enum(options, { error });
}

export function wholeNumber(argument: string, min: number, max: number): z.ZodNumber {
  const error = `${CODE} ${argument} takes a whole number from ${min} to ${max}.`;
...
export function tagNames(argument: string, least: number, most: number): z.ZodArray<z.ZodString> {
  const error = `${CODE} ${argument} takes between ${least} and ${most} tag names.`;
  return z.array(tagName(argument, 120), { error }).min(least, error).max(most, error);
}
